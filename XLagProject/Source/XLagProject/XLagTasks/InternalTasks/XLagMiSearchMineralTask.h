...
{
public:
	XLagMiSearchMineralTask(std::shared_ptr<ITerrainMapAccessor> map, int x, int y, const int32 mineralId)
	...
	virtual void Execute(ACharacter *npc, XLagNPCTaskContext* context, float DeltaTime, int subLevel) override
	{
		...
		auto isFinished = miner->SearchMineral(mapCell, MineralId, DeltaTime);

...
class XLagFmCultivateTask : public XLagNPCTaskBase
{
public:
	XLagFmCultivateTask(std::shared_ptr<ITerrainMapAccessor> map, int x, int y)
		:Map(map), X(x), Y(y)
	{
	...
	virtual void Execute(ACharacter *npc, float DeltaTime) override
	{
		...
		auto isFinished = farmer->Cultivate(mapCell, DeltaTime);
		if (isFinished)
		{
			Completed = true;
		}
